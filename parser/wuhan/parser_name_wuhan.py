...
def extract_social_security_wuhan(pdf_file)-> List[SocialSecurityUser]:
    """
    ...
    # 序号 姓名 身份证号 个人编号
    # 1 彭文凯 420115199202148337 10048880229
    #
    s = r'^(\d+)\s+([\u4e00-\u9fa5]+)\s+(\d{17}[\dXx])\s+(\d+)\s+(.*)$'
...
class WuHanParser(AbstractParser):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def parse_file(self, file_name: str) -> List[SocialSecurityUser]:
        return extract_social_security_wuhan( file_name )
