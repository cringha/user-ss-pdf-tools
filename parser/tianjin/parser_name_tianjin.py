...
def extract_social_security_tianjin(pdf_file):
    """
    ...
    #
    pattern = re.compile(r'^(\d+)\s+([^\s]+)\s+(\d{17}[\dXx])\s+([^\s]+)\s+(\d{6})\s+(\d{6})\s+(\d+)$')
    output = []
    items = common_social_security_extractor(pdf_file, pattern, 1)
    for item in items:
        num, name, id_num, _, _, _, _ = item
...
class TianJinParser(AbstractParser):
    ...
    def parse_file(self, file_name: str) -> List[SocialSecurityUser]:
        return extract_social_security_tianjin(file_name)

    def snapshot_user(self, file_name: str, user_name: str, target_name: str = None):
        return self.snapshot_user_base(file_name, user_name, target_name, 10, 10, 15, False)
